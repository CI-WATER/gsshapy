...
of the files are GRASS ASCII maps and some of the data in other files are spatial
in nature (though not in a standard format like a shp file).

The files that are supported are represented in GsshaPy by classes. There
...
In most cases, a majority of the information in each file is stored in the database tables 
associated with the supporting *table* classes. The *file* class ``read()`` and ``write()``
methods orchestrate the reading and writing of data using the supporting *table* classes.
